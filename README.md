...
`env`, `which grep`, `false ; echo $?`, `err 2> /tmp/err.txt`, `hello test`,
`args one two`, `fastfetch`, `sysctl -a`, `find /proc/sys`, `hexdump /disk/bootsector.bin`, `readelf /bin/readelf.elf`, `sha256sum /etc/os-release`, `cksum /etc/hostname`, `fold -w 12 /proc/version`, `printf '%s:%d:%x\n' IanOS 42 255`, `dd if=/etc/hostname of=/tmp/dd-host bs=6 count=1`, `printf 'alpha beta\n' | xargs echo prefix`, `yes IanOS count=3`, `od -t x1 /etc/hostname`, `/bin/which.elf grep`, `cmp /etc/hostname /proc/sys/kernel/hostname`, `strings /proc/version`, `nl /etc/os-release`, `tr IOS ios /etc/os-release`, `sed s/IanOS/IanOS-dev/ /etc/os-release`, `cut -d = -f 2 /etc/os-release`, `paste /etc/hostname /proc/sys/kernel/hostname`, `rev /etc/hostname`, `seq 3`, `expr 7 + 5`, `date`, or `/bin/cat.elf /bin/args.elf`. Input is echoed; Backspace,
...
- `hello.elf`, `args.elf`, `cat.elf`, `ls.elf`, `hostname.elf`, `fastfetch.elf`, `sysctl.elf`, `find.elf`, `hexdump.elf`, `readelf.elf`, `sha256sum.elf`, `cksum.elf`, `fold.elf`, `printf.elf`, `dd.elf`, `cmp.elf`, `strings.elf`, `nl.elf`, `tr.elf`, `sed.elf`, `cut.elf`, `paste.elf`, `rev.elf`, `seq.elf`, `expr.elf`, `id.elf`, `stat.elf`, `sort.elf`, `uniq.elf`, `tee.elf`, `ids.elf`, `ctx.elf`, `fds.elf`, `lsof.elf`, `fdinh.elf`, `ln.elf`, `truncate.elf`, `blk.elf`, `mount.elf`, `df.elf`, `du.elf`, `lsblk.elf`, `pipeinfo.elf`, `devio.elf`, `tty.elf`, `stty.elf`, `ttyread.elf`, `clear.elf`, `kill.elf`, `pgrep.elf`, and `killall.elf` are built, loaded by the bootloader as boot modules, exposed through VFS, spawnable into runnable process records with argv metadata and process-group identity, executable through the scheduler-backed wait path, able to read arguments, files, VFS metadata, inherited stdin/stdout, current process/thread identity, current user context, descriptor-table snapshots, syscall accounting, boot block-device metadata, mount records, filesystem usage, pipe-table snapshots, character-device I/O, terminal input-mode control, terminal clearing, canonical terminal stdin reads, RAM-backed hard-link and truncate syscalls, bounded hex/ASCII file output, ELF64 header inspection, SHA-256 and CRC32 content hashing, bounded line wrapping, formatted script output, block-sized file copying, stdin-driven child spawning, dual-file comparison, printable string extraction, numbered, translated, substituted, field-selected, line-pasted, reversed text output, numeric sequence generation, integer expression evaluation, and process-control syscalls, and able to return to the shell through `Exit`.
