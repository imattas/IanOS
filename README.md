...
`args one two`, `fastfetch`, `sysctl -a`, `find /proc/sys`, `hexdump /disk/bootsector.bin`, `readelf /bin/readelf.elf`, `sha256sum /etc/os-release`, `cmp /etc/hostname /proc/sys/kernel/hostname`, `strings /proc/version`, `nl /etc/os-release`, `tr IOS ios /etc/os-release`, `sed s/IanOS/IanOS-dev/ /etc/os-release`, `cut -d = -f 2 /etc/os-release`, `paste /etc/hostname /proc/sys/kernel/hostname`, `rev /etc/hostname`, `seq 3`, `date`, or `/bin/cat.elf /bin/args.elf`. Input is echoed; Backspace,
...
- `hello.elf`, `args.elf`, `cat.elf`, `ls.elf`, `hostname.elf`, `fastfetch.elf`, `sysctl.elf`, `find.elf`, `hexdump.elf`, `readelf.elf`, `sha256sum.elf`, `cmp.elf`, `strings.elf`, `nl.elf`, `tr.elf`, `sed.elf`, `cut.elf`, `paste.elf`, `rev.elf`, `seq.elf`, `id.elf`, `stat.elf`, `sort.elf`, `uniq.elf`, `tee.elf`, `ids.elf`, `ctx.elf`, `fds.elf`, `lsof.elf`, `fdinh.elf`, `ln.elf`, `truncate.elf`, `blk.elf`, `mount.elf`, `df.elf`, `du.elf`, `lsblk.elf`, `pipeinfo.elf`, `devio.elf`, `tty.elf`, `stty.elf`, `ttyread.elf`, `clear.elf`, `kill.elf`, `pgrep.elf`, and `killall.elf` are built, loaded by the bootloader as boot modules, exposed through VFS, spawnable into runnable process records with argv metadata and process-group identity, executable through the scheduler-backed wait path, able to read arguments, files, VFS metadata, inherited stdin/stdout, current process/thread identity, current user context, descriptor-table snapshots, syscall accounting, boot block-device metadata, mount records, filesystem usage, pipe-table snapshots, character-device I/O, terminal input-mode control, terminal clearing, canonical terminal stdin reads, RAM-backed hard-link and truncate syscalls, bounded hex/ASCII file output, ELF64 header inspection, SHA-256 content hashing, dual-file comparison, printable string extraction, numbered, translated, substituted, field-selected, line-pasted, reversed text output, numeric sequence generation, and process-control syscalls, and able to return to the shell through `Exit`.
