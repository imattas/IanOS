...
`args one two`, `fastfetch`, `sysctl -a`, `find /proc/sys`, `hexdump /disk/bootsector.bin`, `readelf /bin/readelf.elf`, `sha256sum /etc/os-release`, `cksum /etc/hostname`, `fold -w 12 /proc/version`, `printf '%s:%d:%x\n' IanOS 42 255`, `dd if=/etc/hostname of=/tmp/dd-host bs=6 count=1`, `printf 'alpha beta\n' | xargs echo prefix`, `yes IanOS count=3`, `od -t x1 /etc/hostname`, `/bin/which.elf grep`, `cal 7 2026`, `cmp /etc/hostname /proc/sys/kernel/hostname`, `strings /proc/version`, `nl /etc/os-release`, `tr IOS ios /etc/os-release`, `sed s/IanOS/IanOS-dev/ /etc/os-release`, `cut -d = -f 2 /etc/os-release`, `paste /etc/hostname /proc/sys/kernel/hostname`, `rev /etc/hostname`, `seq 3`, `expr 7 + 5`, `date`, or `/bin/cat.elf /bin/args.elf`. Input is echoed; Backspace,
