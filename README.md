...
system summary with the IanOS icon beside OS and Mattas kernel details before the prompt. Type
commands in that window, for example `help`, `ls`, `export EDITOR=hksh`,
`env`, `which grep`, `false ; echo $?`, `err 2> /tmp/err.txt`, `hello test`,
`args one two`, `date`, or `/bin/cat.elf /bin/args.elf`. Input is echoed; Backspace,
