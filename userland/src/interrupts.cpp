...
bool stream_interrupts(uint64_t& bytes) {
    bytes = 0;
    static const char path[] = "/proc/interrupts";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[interrupts] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[interrupts] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[interrupts] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_interrupts(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[interrupts] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
