...
bool stream_sched_debug(uint64_t& total) {
    total = 0;
    const char* path = "/proc/sched_debug";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[scheddebug] ", "open error ", opened.error);
        return false;
    }
    hybrid::user::write_text("[scheddebug] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[scheddebug] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_sched_debug(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[scheddebug] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
