...
void build_pid_cwd_path(uint64_t pid, char (&path)[32]) {
    uint64_t cursor = 0;
    hybrid::user::append_text(path, sizeof(path), cursor, "/proc/");
    append_decimal(path, sizeof(path), cursor, pid);
    hybrid::user::append_text(path, sizeof(path), cursor, "/cwd");
}
...
bool stream_file(const char* path, uint64_t& bytes) {
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[procwd] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text_line("[procwd] ", "path ", path);
    hybrid::user::write_text("[procwd] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[procwd] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    static const char self_path[] = "/proc/self/cwd";
    ...
    if (get_arg(1, arg)) {
        ...
        if (!parse_pid(arg.value, pid)) {
            hybrid::user::write_line("[procwd] usage procwd [pid]");
            hybrid::user::exit(2);
        }
        build_pid_cwd_path(pid, pid_path);
        path = pid_path;
    }

    uint64_t bytes = 0;
    if (!stream_file(path, bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[procwd] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
