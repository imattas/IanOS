...
constexpr uint64_t kSha384Initial[8] = {
...
void init(Sha384& ctx) {
    for (uint64_t i = 0; i < 8; ++i) ctx.state[i] = kSha384Initial[i];
...
void add_bytes(Sha384& ctx, uint64_t length) {
    const uint64_t old = ctx.bytes_low;
    ctx.bytes_low += length;
    if (ctx.bytes_low < old) ++ctx.bytes_high;
}

void transform(Sha384& ctx, const unsigned char* block) {
    uint64_t w[80];
...
void update(Sha384& ctx, const unsigned char* data, uint64_t length) {
...
void finish(Sha384& ctx, unsigned char digest[48]) {
    ...
    transform(ctx, ctx.block);
    for (uint64_t i = 0; i < 6; ++i) store_be64(digest + i * 8, ctx.state[i]);
}
...
void write_digest(const unsigned char digest[48], const char* path) {
    char line[160];
    uint64_t cursor = 0;
    hybrid::user::append_text(line, sizeof(line), cursor, "[sha384sum] ");
...
extern "C" [[noreturn]] void _start() {
    ...
    if (has_path) {
        ...
        if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
            hybrid::user::write_hex_line("[sha384sum] ", "open error ", opened.error);
            hybrid::user::exit(2);
        }
        fd = opened.value;
    ...
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[sha384sum] ", "read error ", read.error);
            hybrid::user::exit(3);
        }
    ...
    hybrid::user::write_hex_line("[sha384sum] ", "bytes ", ctx.bytes_low);
    hybrid::user::exit(0);
}
