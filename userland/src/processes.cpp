...
bool stream_processes(uint64_t& bytes) {
    bytes = 0;
    static const char path[] = "/proc/processes";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[processes] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[processes] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[processes] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_processes(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[processes] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
