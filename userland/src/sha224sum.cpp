...
constexpr uint32_t kSha224Initial[8] = {
...
void init(Sha224& ctx) {
    for (uint64_t i = 0; i < 8; ++i) ctx.state[i] = kSha224Initial[i];
    for (uint64_t i = 0; i < sizeof(ctx.block); ++i) ctx.block[i] = 0;
    ctx.block_used = 0;
    ctx.bytes = 0;
}

void transform(Sha224& ctx, const unsigned char* block) {
    uint32_t w[64];
...
void update(Sha224& ctx, const unsigned char* data, uint64_t length) {
    ctx.bytes += length;
    uint64_t consumed = 0;
...
void finish(Sha224& ctx, unsigned char digest[28]) {
...
void write_digest(const unsigned char digest[28], const char* path) {
    char line[128];
    uint64_t cursor = 0;
    hybrid::user::append_text(line, sizeof(line), cursor, "[sha224sum] ");
...
extern "C" [[noreturn]] void _start() {
    ...
    if (has_path) {
        ...
        if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
            hybrid::user::write_hex_line("[sha224sum] ", "open error ", opened.error);
            hybrid::user::exit(2);
        }
        fd = opened.value;
    ...
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[sha224sum] ", "read error ", read.error);
            hybrid::user::exit(3);
        }
    ...
    hybrid::user::write_hex_line("[sha224sum] ", "bytes ", ctx.bytes);
    hybrid::user::exit(0);
}
