...
void build_pid_io_path(uint64_t pid, char (&path)[32]) {
    uint64_t cursor = 0;
    hybrid::user::append_text(path, sizeof(path), cursor, "/proc/");
    append_decimal(path, sizeof(path), cursor, pid);
    hybrid::user::append_text(path, sizeof(path), cursor, "/io");
}
...
bool stream_file(const char* path, uint64_t& bytes) {
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[procio] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text_line("[procio] ", "path ", path);
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[procio] ", "read error ", read.error);
            hybrid::user::syscall(hybrid::SyscallNumber::VfsClose, opened.value);
            return false;
        }
        if (read.value == 0) break;
        bytes += read.value;
        hybrid::user::write_text("[procio] ");
...
extern "C" [[noreturn]] void _start() {
    static const char self_path[] = "/proc/self/io";
    ...
    if (get_arg(1, arg)) {
        ...
        if (!parse_pid(arg.value, pid)) {
            hybrid::user::write_line("[procio] usage procio [pid]");
            hybrid::user::exit(2);
        }
        build_pid_io_path(pid, pid_path);
        path = pid_path;
    }

    uint64_t bytes = 0;
    if (!stream_file(path, bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[procio] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
