...
bool stream_tty_stats(uint64_t& bytes) {
    bytes = 0;
    static const char path[] = "/proc/tty/summary";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[ttystat] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[ttystat] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[ttystat] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_tty_stats(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[ttystat] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
