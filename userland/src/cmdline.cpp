...
bool stream_cmdline(uint64_t& bytes) {
    bytes = 0;
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[cmdline] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[cmdline] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[cmdline] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_cmdline(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[cmdline] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
