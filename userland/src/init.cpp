...
void command_help(const char*) {
    ...
    shell_line("", "external: hello args cat ls uname hostname free uptime date cal dmesg ps pwd env printenv sysinfo fastfetch sysctl id ids ctx echo sleep true false touch append rm cp mv dd wc grep tee mkdir rmdir err stat file whoami basename dirname head tail test sort uniq find hexdump readelf sha256sum cmp cksum fold printf strings nl tr sed cut paste rev tac seq expr xargs yes od which sh duptest fds lsof fdinh ln readlink realpath truncate blk mount df du lsblk pipeinfo kill killall pgrep uyield ubusy slowcat burst loop devio tty stty ttyread clear");
...
const char* const kExternalCommands[] = {
    ...
    "stat", "file", "whoami", "basename", "dirname", "head", "tail", "test", "sort", "uniq", "find", "hexdump", "readelf", "sha256sum", "cmp", "cksum", "fold", "printf", "strings", "nl", "tr", "sed", "cut", "paste", "rev", "tac", "seq", "expr", "xargs", "yes", "od", "which", "sh", "duptest", "fds", "lsof", "fdinh", "ln", "readlink", "realpath", "truncate", "blk", "mount", "df", "du", "lsblk", "pipeinfo", "kill", "killall", "pgrep", "uyield", "ubusy", "slowcat", "burst", "loop", "devio", "tty", "stty", "ttyread", "clear",
};
...
bool recovery_passthrough_allowed(const char* command) {
    return streq(command, "cat") || streq(command, "cd") || streq(command, "df") ||
        streq(command, "dmesg") || streq(command, "devices") || streq(command, "fb") ||
