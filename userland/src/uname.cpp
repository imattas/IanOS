...
extern "C" [[noreturn]] void _start() {
    ...
    auto result = syscall(hybrid::SyscallNumber::GetSystemInfo, reinterpret_cast<uint64_t>(&info));
    if (result.error != hybrid::kSyscallErrorNone) {
        write_line("[uname] system info error");
        syscall(hybrid::SyscallNumber::Exit, 1);
    }
    write_text("[uname] ");
    write_text(info.sysname);
    write_text(" ");
    write_text(info.release);
    write_text(" ");
    write_text(info.machine);
    write_text(" ");
    write_text(info.boot_mode);
    write_text(" ");
    write_line(info.kernel_type);

    char line[96];
    uint64_t cursor = 0;
    append_text(line, sizeof(line), cursor, "[uname] modules ");
    append_hex(line, sizeof(line), cursor, info.boot_module_count);
    write_line(line);
