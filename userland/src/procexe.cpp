...
void build_pid_exe_path(uint64_t pid, char (&path)[32]) {
    uint64_t cursor = 0;
    hybrid::user::append_text(path, sizeof(path), cursor, "/proc/");
    append_decimal(path, sizeof(path), cursor, pid);
    hybrid::user::append_text(path, sizeof(path), cursor, "/exe");
}
...
bool stream_file(const char* path, uint64_t& bytes) {
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[procexe] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text_line("[procexe] ", "path ", path);
    hybrid::user::write_text("[procexe] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[procexe] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    ...
    if (get_arg(1, arg)) {
        ...
        if (!parse_pid(arg.value, pid)) {
            hybrid::user::write_line("[procexe] usage procexe [pid]");
            hybrid::user::exit(2);
        }
        build_pid_exe_path(pid, pid_path);
        path = pid_path;
    }

    uint64_t bytes = 0;
    if (!stream_file(path, bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[procexe] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
