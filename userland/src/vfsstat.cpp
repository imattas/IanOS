...
bool stream_vfs_stats(uint64_t& bytes) {
    bytes = 0;
    static const char path[] = "/proc/fs/vfs";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[vfsstat] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[vfsstat] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[vfsstat] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_vfs_stats(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[vfsstat] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
