...
int main_result() {
    auto opened = hybrid::user::syscall(hybrid::SyscallNumber::Open,
                                        reinterpret_cast<uint64_t>("/proc/net/route"),
                                        sizeof("/proc/net/route"));
    if (opened.error != hybrid::kSyscallErrorNone) {
        hybrid::user::write_hex_line("[route] ", "open error ", opened.error);
        return 1;
    }
    ...
    while (true) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            hybrid::user::write_hex_line("[route] ", "read error ", read.error);
            return 2;
        }
