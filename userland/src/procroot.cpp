...
void build_pid_root_path(uint64_t pid, char (&path)[32]) {
    uint64_t cursor = 0;
    hybrid::user::append_text(path, sizeof(path), cursor, "/proc/");
    append_decimal(path, sizeof(path), cursor, pid);
    hybrid::user::append_text(path, sizeof(path), cursor, "/root");
}
...
bool stream_file(const char* path, uint64_t& bytes) {
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[procroot] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text_line("[procroot] ", "path ", path);
    hybrid::user::write_text("[procroot] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[procroot] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    static const char self_path[] = "/proc/self/root";
    ...
    if (get_arg(1, arg)) {
        ...
        if (!parse_pid(arg.value, pid)) {
            hybrid::user::write_line("[procroot] usage procroot [pid]");
            hybrid::user::exit(2);
        }
        build_pid_root_path(pid, pid_path);
        path = pid_path;
    }

    uint64_t bytes = 0;
    if (!stream_file(path, bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[procroot] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
