...
bool stream_heapinfo(uint64_t& total) {
    total = 0;
    const char* path = "/proc/heapinfo";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[heapinfo] ", "open error ", opened.error);
        return false;
    }
    hybrid::user::write_text("[heapinfo] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[heapinfo] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_heapinfo(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[heapinfo] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
