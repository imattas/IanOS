...
bool stream_iomem(uint64_t& total) {
    total = 0;
    const char* path = "/proc/iomem";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[iomem] ", "open error ", opened.error);
        return false;
    }
    hybrid::user::write_text("[iomem] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[iomem] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_iomem(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[iomem] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
