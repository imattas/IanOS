...
bool stream_buddyinfo(uint64_t& total) {
    total = 0;
    const char* path = "/proc/buddyinfo";
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[buddyinfo] ", "open error ", opened.error);
        return false;
    }
    hybrid::user::write_text("[buddyinfo] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[buddyinfo] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_buddyinfo(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[buddyinfo] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
