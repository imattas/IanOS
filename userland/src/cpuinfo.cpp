...
bool stream_cpuinfo(uint64_t& bytes) {
    bytes = 0;
    ...
    if (opened.error != hybrid::kSyscallErrorNone || opened.value < 3) {
        hybrid::user::write_hex_line("[cpuinfo] ", "open error ", opened.error);
        return false;
    }

    hybrid::user::write_text("[cpuinfo] ");
    for (;;) {
        ...
        if (read.error != hybrid::kSyscallErrorNone) {
            ...
            hybrid::user::write_hex_line("[cpuinfo] ", "read error ", read.error);
...
extern "C" [[noreturn]] void _start() {
    uint64_t bytes = 0;
    if (!stream_cpuinfo(bytes)) hybrid::user::exit(1);
    hybrid::user::write_hex_line("[cpuinfo] ", "bytes ", bytes);
    hybrid::user::exit(0);
}
