...
void write_hex_line(const char* label, uint64_t value) {
    char line[96];
    uint64_t cursor = 0;
    append_text(line, sizeof(line), cursor, "[free] ");
...
extern "C" [[noreturn]] void _start() {
    ...
    if (result.error != hybrid::kSyscallErrorNone) {
        write_hex_line("error ", result.error);
        syscall(hybrid::SyscallNumber::Exit, 1);
    }
    write_hex_line("total pages ", stats.total_pages);
    write_hex_line("used pages ", stats.used_pages);
    write_hex_line("free pages ", stats.free_pages);
    write_hex_line("usable bytes ", stats.usable_bytes);
